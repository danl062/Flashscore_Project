            ...
            if not venue_exists_in_db(team, cursor) or not country_exists_in_db(team, cursor):
                ...
                if team_api_name:
                    ...
                    if team_id:
                        ...
                        logging.warning(f"Can't find ID for this team {team}")
                else:
                    logging.warning(f"Team {team} not found in the API")
            else:
                logging.info(f"Venue or country for team {team} already exists in the database")
        except Exception as e:
            logging.error(f"Error processing team {team}: {e}")
            pass
    conn.commit()
    conn.close()
